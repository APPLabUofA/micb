...
figure('Position',[25,25,1700,900]); 
widthHeight = ceil(sqrt(nsubs));
...
%% Looking at the correct trial gabor distances as a function of condition
...
for i_subs = 1:nsubs
    ...
    if i_subs == 1
        SOA_dist_con_group = zeros(nsubs,length(soas));
        SOA_dist_con_mean = length(soas);
        SOA_dist_con_std = length(soas);
        SOA_dist_exp_group = zeros(nsubs,length(soas));
        SOA_dist_exp_mean = length(soas);
        SOA_dist_exp_std = length(soas);
    end

    for i = 1:lengthy
        if isnan(out_incorrect_gabor(i)) ~= true 
            if trialList(i,3) == 0 && isnan(out_incorrect_gabor(i)) == true
                count1 = count1 + 1;
            ...
            elseif trialList(i,3) == 1 && isnan(out_incorrect_gabor(i)) == true
                count2 = count2 + 1;
    ...
    for ii = 1:length(soas)
        SOA_dist_con_mean(ii) = mean(par_str2(i_subs).Incor_Con(5,par_str2(i_subs).Incor_Con(6,1:count1) == soas(ii)));
        SOA_dist_con_std(ii) = std(par_str2(i_subs).Incor_Con(5,par_str2(i_subs).Incor_Con(6,1:count1) == soas(ii)));
    end
    for ii = 1:length(soas)
        SOA_dist_exp_mean(ii) = mean(par_str2(i_subs).Incor_Exp(5,par_str2(i_subs).Incor_Exp(6,1:count2) == soas(ii)));
    ...
    hold off;
        ...
        ylim([.01 2])
        title(current_sub)
    SOA_dist_con_group(i_subs,:) = SOA_dist_con_mean;
    SOA_dist_exp_group(i_subs,:) = SOA_dist_exp_mean;
end  
figure;
	errorbar(soas, mean(SOA_dist_con_group,1), std(SOA_dist_con_group,[],1) / sqrt(nsubs),'r');
    errorbar(soas, mean(SOA_dist_exp_group,1), std(SOA_dist_exp_group,[],1) / sqrt(nsubs),'b');
    legend({'Control','Experimental'});
    xlim([min(soas) max(soas)]); 
    set(gca,'XTick',min(soas):1:max(soas))
    xlabel('Gabor First < -- SOA (frames) -- > Gabor After')
...
%% Incorrect trial gabor orientation as a function of counterclockwise and clockwise gabor orientations
...
%% Incorrect trial gabor orientation (30 degree) as a function of SOA
...
for i_subs = 1:nsubs
    ...
    par_str3(i_subs).Incor = zeros(6,lengthy);
...
%% Incorrect trial gabor orientation as a function of surrounding gabor orientation (ignoring side)
